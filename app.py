...
with col1:
    fill_color = st.color_picker(
        "QR Colour",
        "#000000", help="Click to select a color. Default is black."
    )

with col2:
    back_color = st.color_picker(
        "Background Colour",
        "#FFFFFF", help="Click to select a color. Default is white."
    )
...
error_level = st.selectbox(
    ...
    index=3,
    help="Select the error correction level. Higher levels allow for more damage to the QR code while still being scannable, but may reduce the amount of data that can be stored."
)
...
box_size = st.slider(
    ...
    max_value=20,
    value=10,
    help="Adjust the size of the QR code. Higher values create larger QR codes with more detail, but may not be compatible with all scanners."
)

logo_file = st.file_uploader(
    "Upload Logo (Optional)",
    type=["png", "jpg", "jpeg"],
    help="Upload a logo image to embed in the center of the QR code. Supported formats: PNG, JPG, JPEG."
)
...
filename = st.text_input(
    "Download Filename",
    value="ricktech_qr",
    help="Enter a name for your QR code file (without extension). Default is 'ricktech_qr'."
)
